          ...
          {[
            ...
            <div key={q.key} className="glass-card flex-between" style={{ padding: '1rem 1.5rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <button 
                  className={`btn ${answers[q.key] === true ? 'btn-primary' : 'btn-secondary'}`}
                  onClick={() => handleAnswer(q.key, true)}
                >
                  Yes
                </button>
                <button 
                  className={`btn ${answers[q.key] === false ? 'btn-primary' : 'btn-secondary'}`}
                  onClick={() => handleAnswer(q.key, false)}
                  style={{ background: answers[q.key] === false ? 'var(--danger)' : '' }}
                >
                  No
                </button>
              </div>
