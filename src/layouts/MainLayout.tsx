...
const navItems = [
  ...
  { path: '/step-1', label: '1. Applicability', icon: <Stethoscope size={20} /> },
  { path: '/step-2', label: '2. Classification', icon: <Scale size={20} /> },
  { path: '/step-3', label: '3. QMS Builder', icon: <ClipboardList size={20} /> },
  { path: '/sw-planning', label: 'IEC 62304 Planning', icon: <ListChecks size={20} /> },
  { path: '/sw-architecture', label: 'SW Architecture', icon: <MonitorPlay size={20} /> },
  { path: '/step-4', label: '4. Clinical Data', icon: <Microscope size={20} /> },
  { path: '/step-5', label: '5. Tech File', icon: <FileText size={20} /> },
  { path: '/step-6', label: '6. Notified Body', icon: <Building2 size={20} /> },
  { path: '/step-7', label: '7. EU Reqs', icon: <Globe size={20} /> },
  { path: '/step-8', label: '8. CE Marking', icon: <Award size={20} /> },
  { path: '/step-9', label: '9. Post-Market', icon: <Activity size={20} /> },
];

export default function MainLayout() {
  const { products, activeProductId, setActiveProduct } = useProductContext();
        ...
        <header className="top-header">
          <div className="flex-between">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              ...
              <button className="btn btn-primary" style={{ border: '1px solid rgba(255,255,255,0.3)' }}>Start Review</button>
            </div>
          </div>
        </header>
